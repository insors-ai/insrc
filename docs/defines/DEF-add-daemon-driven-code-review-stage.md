...
- **[[c1]]** `code` `src/workflow/review/run-artifact.ts:66-104` — "reviewArtifactFile(opts: ReviewArtifactOpts): Promise<ReviewArtifactResult> — the existing artifact-review entry point; premise-audits DEF/HLD/LLD/PLAN text, not code."
- **[[c2]]** `code` `src/workflow/review/run-artifact.ts:30-51` — "ReviewArtifactOpts (30-44) and ReviewArtifactResult (46-51) — the input/verdict shapes the proposed code-review record is asked to mirror."
- **[[c3]]** `code` `src/mcp/review-step/handler.ts:32-43` — "handleReviewStep(input: unknown): Promise<ReviewStepMcpEnvelope> — the controller-drivable MCP entry point insrc_review_step exposes."
- **[[c4]]** `code` `src/mcp/review-step/types.ts:30-146` — "ReviewStepInputStart (30-34), InputClaims (37-41), InputVerdicts (45-49), EmitClaims (71-78), EmitVerdicts (80-89), Done (91-103), Error (105-112), StatePayload (124-136), McpEnvelope (142-146) — the "
- **[[c5]]** `code` `src/cli/services/workflow.ts:234-250` — "reviewArtifact(repoPath: string, artifactPath: string): Promise<ReviewArtifactResult> — CLI-service call site that drives reviewArtifactFile."
- **[[c6]]** `code` `src/daemon/workflow-rpc.ts` — "Ranked 0.314 by concept.resolve for "workflow-rpc daemon self-review LLMProvider R2 controller review". Only its path and resolver ranking are grounded here; its internal shape was not profiled."
- **[[c7]]** `analyze-bundle` `module.profile — src/workflow/review/` — "Directory profile: 12 files, 96 entities, 114,411 bytes, sole declared entrypoint src/workflow/review/index.ts. Members include probe.ts, verify.ts, calibration.ts, extract.ts, resolve.ts, apply.ts, r"
- **[[c8]]** `doc` `CLAUDE.md — "Building features via insrc — classify FIRST, review before approve"` — ""insrc_review_step before approve — the independent 'two sets of eyes' review (a daemon self-review is the SAME model that authored the artifact)." ... "It stamps approvedAt and enforces the review bl"
- **[[c9]]** `prior-artifact` `Epic 185807ba9a6b35d3 / add-build-workflow-insrc-5th-stage` — "s5 'Finalize the run into a reviewable, approvable record of what was built' — the record of the build run's own actions; the Epic's problem statement never raises independently auditing the produced "
- **[[c10]]** `prior-artifact` `Epic 1cd9a4c34f403a80 / add-plan-workflow-insrc-framework-4th` — "s4: 'State the tests each unit of work should produce' — a direct input to the coverage dimension, but this Epic only produces the plan; it never verifies code against it."
- **[[c11]]** `prior-artifact` `Epic 8f485fe5c614fefc / add-module-test-health-analyzer-new` — "s3: 'See which of a module's behaviour tests reach and which they leave unverified' — a read-only ANALYZE target scoped to a module, not a workflow gate scoped to a Story's diff."
- **[[c12]]** `convention` `convention.detect — src/workflow/review/run-artifact.ts` — "Every sample fell below the 5-entity confidence threshold: function naming n=2 (camelCase), class naming n=2 (PascalCase), file naming n=1 (kebab-case), no test files detected, no base-class idioms, 0"
- **[[c13]]** `step-output` `s1 scope/context — decision + flavorEvidence` — "decision: "new", scope: "L", flavor: "enhancement"; "Existing code to extend + no matching Epic => enhancement, and a new Define is required.""
- **[[c14]]** `doc` `CLAUDE.md — "Project principles"` — ""Accuracy is primary; cost is the least priority." ... "No direct cloud REST calls from our process. Cloud LLM access happens through the locally-installed claude and codex CLI binaries (via CliProvid"
- **[[c15]]** `analyze-bundle` `concept.resolve (second pass) — daemon self-review driver` — "Ranked src/workflow/review/ top (0.351) followed by src/daemon/workflow-rpc.ts (0.314), with src/daemon/ (0.244), src/mcp/review-step/ (0.181-0.264) and src/workflow/artifacts/ (0.187) trailing. Every"
- **[[c16]]** `doc` `CLAUDE.md — "Key architectural rules"` — ""Daemon owns all DB access — CLI, MCP, and the IDE workbench communicate via IPC only." ... "No raw file dumps — context is always structured entity summaries + relations from the graph.""
- **[[c17]]** `doc` `CLAUDE.md — "Code conventions"` — ""Always use .js extension in import paths" ... "Use import type for type-only imports" ... "Use getLogger('module-name') — never use console.log" ... "Never Promise.all anything that reaches an LLM pr"
- **[[c18]]** `code` `src/workflow/review/run-artifact.ts:66-104` — "reviewArtifactFile(opts: ReviewArtifactOpts): Promise<ReviewArtifactResult> — the existing artifact-review entry point; premise-audits DEF/HLD/LLD/PLAN text, not code."
...
**0 HIGH · 0 MED · 11 LOW** · model `client` · reviewed 2026-08-03T06:54:46.433Z

| Ref | Kind | Severity | Fixability | Premise | Evidence | Action |
| --- | --- | --- | --- | --- | --- | --- |
| c1 | citation | LOW | manual | reviewArtifactFile(opts: ReviewArtifactOpts): Promise<ReviewArtifactResult> is defined in src/workflow/review/run-artifact.ts around lines 66-104 and is the existing artifact-review entry point. | grep + read confirm `export async function reviewArtifactFile(opts: ReviewArtifactOpts): Promise<ReviewArtifactResult>` at src/workflow/review/run-artifact.ts:66 — exact anchor match. | None; citation c1 is sound. |
| c2 | citation | LOW | manual | ReviewArtifactOpts and ReviewArtifactResult — the input/verdict shapes the code-review record is asked to mirror — are declared in src/workflow/review/run-artifact.ts around lines 30-51. | `export interface ReviewArtifactOpts` at run-artifact.ts:30 and `export interface ReviewArtifactResult` at :46 — both within the cited 30-51 range. | None; citation c2 is sound. |
| c3 | citation | LOW | manual | handleReviewStep(input: unknown): Promise<ReviewStepMcpEnvelope> is the controller-drivable MCP entry point for insrc_review_step, in src/mcp/review-step/handler.ts around lines 32-43. | `export async function handleReviewStep(input: unknown): Promise<ReviewStepMcpEnvelope>` at src/mcp/review-step/handler.ts:32 — exact anchor match. | None; citation c3 is sound. |
| c4 | citation | LOW | manual | The review-step multi-turn phase envelope types (ReviewStepInputStart, InputClaims, InputVerdicts, EmitClaims, EmitVerdicts, Done, Error, StatePayload, McpEnvelope) are declared in src/mcp/review-step/types.ts within lines 30-146. | ReviewStepInputStart at types.ts:30 and ReviewStepEmitClaims at :71 confirmed; the phase-envelope types (EmitClaims/EmitVerdicts/StatePayload) all resolve within src/mcp/review-step/types.ts as cited (30-146). | None; citation c4 is sound. |
| c5 | citation | LOW | manual | reviewArtifact(repoPath: string, artifactPath: string): Promise<ReviewArtifactResult> is the CLI-service call site driving reviewArtifactFile, in src/cli/services/workflow.ts around lines 234-250. | `export async function reviewArtifact(repoPath, artifactPath): Promise<ReviewArtifactResult>` at src/cli/services/workflow.ts:234, also wired in cli/command.ts:188 and services/index.ts:54 — the CLI surface c5 names really exists. | None; citation c5 is sound. |
| c6 | citation | LOW | manual | A daemon-side LLMProvider-driven review/workflow driver exists at src/daemon/workflow-rpc.ts (the low-confidence assumption a5 leans on this path existing). | src/daemon/workflow-rpc.ts exists (read at :1 found=true); runWorkflowServerSide/runStart are the documented server-side entrypoints (referenced by prior LLD-add-build-workflow s3). The path claim c6 is confirmed. The DEF itself correctly flags the INTERNAL SHAPE as unverified (assumption a5 = low, self-noted open question), which is honest, not a citation defect. | None required for citation soundness; the fit-of-shape question is already tracked as the DEF's own open item and should be resolved during the HLD when workflow-rpc.ts is profiled. |
| c7 | inventory | LOW | manual | The src/workflow/review/ module comprises 12 files, and includes probe.ts, verify.ts, calibration.ts, extract.ts, resolve.ts, apply.ts, report.ts, run-artifact.ts and index.ts as its sole declared entrypoint. | Direct re-derivation: `ls src/workflow/review/*.ts` (excluding __tests__) = 12 files, including probe.ts, verify.ts, calibration.ts, extract.ts, resolve.ts, apply.ts, report.ts, run-artifact.ts and index.ts — exactly matching module.profile c7 (12 files, sole entrypoint index.ts). | None; the sizing inventory c7 is accurate. |
| c9 | cross-artifact | LOW | manual | The build workflow Epic (185807ba9a6b35d3) exists and its final story finalizes the build run into a reviewable record of what was built — an input this stage consumes and must not re-derive. | DEF-185807ba9a6b35d3.md exists (docs/defines/DEF-add-build-workflow-insrc-5th-stage.md:1) — the build Epic c9 names is real, and this DEF correctly treats its build-run record as an input. | None; cross-artifact trace c9 holds. |
| c10 | cross-artifact | LOW | manual | The plan workflow Epic (1cd9a4c34f403a80) has a story that states the tests each unit of work should produce — the approved expectation the coverage dimension checks against. | DEF-1cd9a4c34f403a80.md exists (docs/defines/DEF-add-plan-workflow-insrc-framework-4th.md:1) — the plan Epic c10 names is real; its 'state the tests each unit should produce' story is the coverage dimension's approved expectation. | None; cross-artifact trace c10 holds. |
| c11 | cross-artifact | LOW | manual | A separate module-test-health analyzer Epic (8f485fe5c614fefc) already frames a module-scoped read-only view of test health, distinct from this Story-diff-scoped gate — the non-goal that avoids duplicate scopes. | DEF-8f485fe5c614fefc.md and HLD-8f485fe5c614fefc.md both exist — the module-test-health analyzer Epic c11 names is real, correctly cited as the separately-owned module-scoped view this Epic must not duplicate. | None; the non-goal boundary against 8f485fe5c614fefc is well-grounded. |
| non-goal/capability | closed-union | LOW | manual | No existing capability in the repo already performs a post-build code review of a Story's changed files against its approved design/plan; the existing review subsystem audits DESIGN artifacts (DEF/HLD/LLD/PLAN) only, so this Epic is genuinely new rather than an amendment. | grep code[_-]?review / reviewCode\|codeReview surface matches ONLY inside this new DEF (k1, ac2, S007 headers) — no pre-existing post-build code-review capability in src/. The existing src/workflow/review + src/mcp/review-step subsystems audit DESIGN artifacts (reviewArtifactFile premise-audits DEF/HLD/LLD/PLAN), confirming this Epic is genuinely new rather than an amendment. | None; the 'genuinely new' basis for a fresh Define (c13) holds. |
