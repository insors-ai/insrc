...
- `qe4330384` — Item ac3: Six of seven map cleanly. s6's userValue ends '— including for the deployment-design work that is already waiting to consume it', and no criterion covers that consumer: ac1/ac2/ac3/ac4 cover surface parity, daemon-side reads, unregistered-repo failure, and discoverability, none of which tests or even mentions the waiting internal consumer. Either drop the clause or add a criterion grounded in c8.
  - **resolved**: Demote the clause to non-normative motivation — The deployment-design consumer is a separate approved Epic; making its end-to-end consumption an S006 acceptance criterion would couple docgen's acceptance to another Epic's path (that Epic tests its own consumption). Keep the "why now" as clearly-labeled motivation on S006 so the rationale survives, while the acceptance criteria stay co-extensive with what docgen S006 actually delivers — surface parity, daemon-side reads, unregistered-repo failure, discoverability (7/7 clean ac↔value mapping). _(2026-07-30T14:31:09.891Z)_
