...
function ctxWith(overrides: Partial<Services> = {}): { ctx: CommandCtx; spies: Spies } {
	...
	const base: Services = {
		...
		config: {
			show: async () => ({ models: { local: { embeddingDim: 768 }, tiers: { core: { runner: 'cli-claude', model: '' } } } }),
...
test('config list shows known options (default) merged with set values', async () => {
	...
	assert.ok(out.some(l => l.includes('models.local.embeddingDim') && l.includes('= 768') && l.includes('(set)')));
	assert.ok(out.some(l => l.includes('models.tiers.core.runner') && l.includes('cli-claude') && l.includes('(set)')));
	// an unset known option is tagged (default) with its default value
	assert.ok(out.some(l => l.includes('models.local.host') && l.includes('http://localhost:11434') && l.includes('(default)')));
	assert.ok(out.some(l => l.includes('models.local.coreModel') && l.includes('(default)')));
});

test('config list <search> filters options by substring', async () => {
	const { ctx } = ctxWith();
	const out = await runCommand('config list tiers', ctx);
	assert.ok(out.length > 0);
	assert.ok(out.every(l => l.includes('tiers')));
	assert.ok(out.some(l => l.includes('models.tiers.core.runner')));
	// a non-matching known option is excluded
	assert.ok(!out.some(l => l.includes('local.host')));
});

test('config get navigates a dot-path', async () => {
	const { ctx } = ctxWith();
	assert.deepEqual(await runCommand('config get models.local.embeddingDim', ctx), ['models.local.embeddingDim = 768']);
