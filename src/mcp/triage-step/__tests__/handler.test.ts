...
function start(focus: string): TriageEmitClassification {
	const env = handleTriageStep({ phase: 'start', focus, repo: REPO });
...
function classify(state: string, result: Record<string, unknown>): TriageDone {
	const env = handleTriageStep({ phase: 'classify', result, state });
...
test('start: returns emit_classification with a schema, prompt, and state token', () => {
	const out = start('Add a --json flag to the status command');
...
test('start: errors without a focus', () => {
	const env = handleTriageStep({ phase: 'start', repo: REPO });
	assert.equal(env.isError, true);
});

test('feature → standalone design.story with plan; nextCall pre-fills standalone params', () => {
	const s = start('Add a triage router with a classifier and MCP tool');
	const done = classify(s.state, wellFormed('feature'));
...
test('small → standalone design.story, no plan', () => {
	const s = start('Add a config flag');
	const done = classify(s.state, wellFormed('small'));
	assert.equal(done.route.startStage, 'design.story');
	assert.equal(done.route.needsPlan, false);
	assert.match(done.summary, /standalone LLD → build/);
});

test('trivial → build (no LLD), routed to insrc_build_step standalone', () => {
	const s = start('Fix a typo in a log message');
	const done = classify(s.state, wellFormed('trivial'));
...
test('epic → define, full chain, no standalone params', () => {
	const s = start('Build a whole new deployment-design subsystem');
	const done = classify(s.state, wellFormed('epic'));
...
test('classify: rejects an unknown sizeClass emitted by the controller', () => {
	const s = start('x');
	const env = handleTriageStep({ phase: 'classify', result: wellFormed('medium'), state: s.state });
	assert.equal(env.isError, true);
});

test('classify: rejects a tampered/undecodable state token', () => {
	const env = handleTriageStep({ phase: 'classify', result: wellFormed('small'), state: 'not-base64-json!!' });
	assert.equal(env.isError, true);
});
