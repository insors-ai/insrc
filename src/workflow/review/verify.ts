...
const SYSTEM = [
	...
	'',
	'Assign `severity`:',
	'  - HIGH : the premise is verifiably WRONG or INCOMPLETE in a way that changes',
	'           the artifact\'s output or acceptance (e.g. it says "the two X" but the',
	'           grep re-derives three; a cited `file:line` resolves to a different',
	'           entity; a producer the plan targets does not exist).',
	'  - MED  : unverifiable from the evidence, a stale/mis-resolved anchor that is',
	'           not material, or a non-blocking concern.',
