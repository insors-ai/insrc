	...
	async run(ctx) {
		...
		return {
			...
			prompt: [
				...
				'- A Story\'s `localConstraints` ids use the `lc` namespace: `lc1`, `lc2`, … (NOT `c1`). Their `source` still references a citation id (`c1`, `c2`, …).',
				'- `operationalizes` must reference constraint ids from Epic (`kN`) OR the Story\'s `localConstraints` (`lcN`).',
