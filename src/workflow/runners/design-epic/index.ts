...
const frameworkWrite = llmPauseRunner({
	...
	buildPrompt: (ctx) => ({
		prompt: [
			...
			'- OWNERSHIP MUST FIT THE EPIC STORY GRAPH — and you CANNOT amend that graph here.',
			'  A Story may only consume a contract owned by a Story it (transitively) `dependsOn`',
			'  (see each Story\'s `dependsOn` in the Epic below). So choose the OWNER to fit the',
			'  graph, do not attach a consumer that is not already downstream of the owner.',
			'  For a genuinely CROSS-CUTTING contract (a compliance / authorization gate, RBAC, a',
			'  shared case or data model) that Stories in DIFFERENT dependency branches all need,',
			'  do NOT own it at the semantically-natural "topic" Story and then attach unrelated',
			'  consumers — own it at the NEAREST COMMON ANCESTOR every consumer already depends on',
			'  (often the foundational first Story), which makes every consumer trivially downstream.',
			'- SELF-CHECK before emitting: for each sharedContract, confirm every entry in its',
			'  `consumedByStories` transitively `dependsOn` its `ownedByStory` in the Epic graph;',
			'  if not, re-own the contract at a common ancestor (never invent an Epic dependency).',
