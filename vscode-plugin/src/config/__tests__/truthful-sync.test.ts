...
test('package.json contributes.commands includes insrc.settings.refresh alongside the 7 shipped commands', () => {
  ...
  assert.equal(cmds.length, 13); // 7 shipped + refresh + the 2 S004 panel commands + the S003 model-picker command + the dev-chat insrc.chat.open + the S007 insrc.chat.docsReview command
});
